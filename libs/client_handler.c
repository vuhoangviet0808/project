...
void *client_handler(void *socket_desc) {
    ...
    while (1) {
        ...
        if (strcmp(command, "register") == 0) {
            ...
            char recver[BUFFER_SIZE], message[BUFFER_SIZE];
            int sender_id, receiver_id;
            sscanf(password, "%s %s", recver, message );
            for (int i=0; i<MAX_CLIENTS; i++) {
                if(strcmp(clients[i].username, username)==0) sender_id = i;
                if(strcmp(clients[i].username, recver) == 0) receiver_id = i;
            }
            send_private_message(sender_id, receiver_id, message);
            log_message("User %s send message to %s", username, recver);
        }
        else {
